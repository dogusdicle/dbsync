...
class Dbsync {
    ...
    private function getCreateTableSql($conn, $table_name) {
        ...
        $ifnotexists = str_replace('CREATE TABLE', 'CREATE TABLE IF NOT EXISTS', $row[1]);
...
EOT;
    ...
    public function getCompareReport() {
        ...
        var_dump($this->create_tables);

        echo '<h2>Silinecek Tablolar</h2>';

        var_dump($this->delete_tables);

        echo '<h2>Eklenecek Sütunlar</h2>';

        var_dump($this->add_columns);

        echo '<h2>Silinecek Sütunlar</h2>';

        var_dump($this->delete_columns);

        echo '<h2>Düzenlenecek Sütunlar</h2>';

        var_dump($this->modify_columns);
    }
